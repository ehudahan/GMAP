...
metadata$Subsets <- sapply(metadata$sampleID, FUN = function(x) case_when(x %in% samples$`last Pre-symptoms` ~ "Last_Pre_Symptomatic",
                                                                          ...
                                                                          x %in% samples$`samples 0-2 Model` ~ "samples 0-2 Model",
                                                                          TRUE ~ "Else"))
...
gg_list <- list()
subset <- "First_Resolved"
for (subset in unique(metadata$Subsets)) {
  gg_list[[subset]] <- gg +   geom_point(data = metadata %>% filter(Subsets == subset),
                          aes(y = reorder(record_id, age_diag_ap_months),
                              x = visit_age_mo,
                              color = symptoms,
                              shape = Subsets), size = 3, alpha = 1) +
    labs(title = subset)
...
gg_arr <- ggarrange(plotlist = gg_list, ncol = 1, common.legend = T)

# Save
{
  path <- "Figures/"
  fileName <- "SampleMapSubsetsSepFig"
  if (1) {
    ggsave(gg_arr,filename = paste0(path, "PDFs/",fileName, ".pdf"), "pdf", useDingbats=FALSE,
           width = 10, height = 40)
    ggsave(gg_arr,filename = paste0(path, "PNGs/",fileName, ".png"), "png",
           width = 10, height = 40)
  }
