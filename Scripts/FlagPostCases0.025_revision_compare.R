...
cases.tab.filter <- cases.tab %>% 
  ...
  filter(Model.Name %in% c("case lastDiet pro anti","case lastDiet pro"))
...
gg <- ggplot(cases.tab.filter, aes(x = coef, y = Feature, fill = Feature))+
  ...
  scale_fill_manual(values = colors, breaks = names(colors))+
  facet_grid(Subset~Model.Name,scales = "free",space = "free")+
  labs(title = "Control vs Cases",
    ...
    caption = paste0("coef > ", thr, " | qval < ", q_val,
                     ...
                     paste0(fixed_effects[[unique(cases.tab.filter$Model.Name)[1]]],
                            fixed_effects[[unique(cases.tab.filter$Model.Name)[2]]],
                            collapse = ", ")))+
...
# Save
{
  ...
  fileName <- paste0("FlagPostCasesCompare", thr)
  width <- 9
  height <- 3
  if (1) {
