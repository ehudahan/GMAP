...
# filtering
{
  ...
  symp.tab.filter <- symp.tab %>% 
    ...
    filter(Model.Name == "symptoms lastDiet pro anti (Control as reference)")
...
gg <- ggplot(symp.tab.filter, aes(x = coef, y = Feature, fill = Feature))+
  ...
  labs(title = "Control vs Symptoms stage",
       ...
       caption = paste0("coef > ", thr, " | qval < ", q_val, "\n",
                        ...
                        paste0(fixed_effects[[unique(symp.tab.filter$Model.Name)]],collapse = ", ")))+
  theme_bw()+
...
# Save
{
  ...
  fileName <- paste0("FlagPostSympAnti", thr)
  width <- 8
  height <- 4
  if (1) {
