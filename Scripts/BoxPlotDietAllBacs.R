...
if(1){
    rm(list=ls())
    # setwd("~/GMAP/Docs/GMAP_Paper/")
    setwd("/Volumes/ehudda/GMAP/Docs/GMAP_Paper/")
    metadata <- read.delim("Results/TSVs/metadata.tsv",as.is=T, header=T, sep = "\t")
...
# plot detalis
{
    ...
    bacs_diet <- c("g__Bifidobacterium", "g__Clostridium","f__Peptostreptococcaceae_unclsfd", "f__Lachnospiraceae_unclsfd")
...
mm.data %>% filter(is.na(lastDiet)) %>% select(sampleID) %>% unique()
...
g <- ggplot(data_diet, aes(x = lastDiet, y = AbundanceAST)) +
    geom_point(aes(color = Bac),alpha = 0.7, size = 2, position = position_jitterdodge(jitter.width = 0.9, seed = 1)) +
