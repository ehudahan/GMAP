...
if(1){
  ...
  maaslin_results <- read.delim("Results/TSVs/SuppTable1_revision.tsv", header = T)
...
# add subset data (True == In the subset)
{
  ...
  six_plus_months <-mm.data %>% filter(sampleID %in% samples$`samples 9-12 Model`)
...
{
  path <- "Figures/"
  fileName <- "BoxPlotSympLactoProWith6-12"
  if (1) {
